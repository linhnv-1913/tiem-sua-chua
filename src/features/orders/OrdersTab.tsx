...
export default function OrdersTab() {
  const { orders, flavors, addOrder, updateOrder, updateOrderStatus } =
    useAppStore();
  ...
  const handleSaveOrder = (e: React.FormEvent) => {
    ...
    const items: OrderItem[] = Object.entries(orderItems).map(
      ([flavorId, qty]) => ({
        flavorId,
  ...
  const handleEditClick = (order: Order) => {
    ...
    order.items.forEach((item) => {
      editItems[item.flavorId] = item.quantitySets;
    });
      ...
      <div className="flex shrink-0 gap-2 mb-4 bg-pink-50 p-1.5 rounded-2xl">
        {[
          ...
          >
            <span>{f.label}</span>
            <span
              className={`text-[9px] px-1.5 py-0.5 mt-0.5 rounded-full leading-none ${
                statusFilter === f.id
          ...
          filteredOrders.map((order) => {
            ...
            return (
              <React.Fragment key={order.id}>
                ...
                <div
                  className={`bg-white border-2 border-pink-50 p-5 rounded-3xl flex flex-col gap-3 ${order.status === "cancelled" ? "opacity-60" : ""}`}
                >
                  <div className="flex justify-between items-start">
                    <div className="flex items-center gap-4">
                      <div
                        className={`w-12 h-12 rounded-xl flex items-center justify-center text-xl shadow-sm ${order.status === "delivered" ? "bg-green-100 text-green-600" : order.status === "cancelled" ? "bg-gray-100 text-gray-500" : "bg-yellow-100 text-yellow-600"}`}
                      >
                      ...
                      <div>
                        <h3 className="font-bold text-[#5C3D3D] text-base">
                          {order.customerName}
                        </h3>
                  ...
                  <div className="text-sm text-[#4A3732] space-y-1 bg-pink-50/50 p-3 rounded-2xl border border-pink-50">
                    <p className="font-medium text-xs text-gray-500">
                      📍 <span className="text-[#4A3732]">{order.address}</span>
                    </p>
                    <p className="font-medium text-xs text-gray-500">
                      ⏰{" "}
                      <span className="text-[#4A3732]">
                        {new Intl.DateTimeFormat("vi-VN", {
                          dateStyle: "short",
                          timeStyle: "short",
                        }).format(safeDate(order.deliveryDate))}
                      </span>
                    </p>
                    <div className="mt-2 pt-2 border-t border-pink-100/50 border-dashed">
                      {order.items.map((item) => {
                        ...
                        return (
                          <p key={item.flavorId} className="font-medium">
                            • {f?.name}: <b>{item.quantitySets} set</b>
                          </p>
                        );
                      })}
                    </div>
                  ...
                  {order.status === "pending" && (
                    <div className="flex gap-2 justify-end mt-2">
                      <button
                        ...
                        className="px-4 py-2 bg-gray-50 border border-gray-100 text-gray-500 font-bold text-xs rounded-xl active:bg-gray-100"
                      >
                        Từ chối
                      </button>
                      <button
                        onClick={() => handleEditClick(order)}
                        className="px-4 py-2 bg-blue-50 border border-blue-100 text-blue-600 font-bold text-xs rounded-xl active:bg-blue-100"
                      >
                        Chỉnh sửa
                      </button>
                      <button
                        onClick={() => updateOrderStatus(order.id, "delivered")}
                        className="px-4 py-2 bg-green-50 border border-green-100 text-green-600 font-bold text-xs rounded-xl active:bg-green-100"
                      >
                        Hoàn thành
                      </button>
                    </div>
      ...
      {showAdd && (
        <div className="absolute inset-0 bg-white z-20 flex flex-col animate-in slide-in-from-bottom h-[100dvh] sm:h-full overflow-hidden sm:rounded-[2.5rem]">
          <div className="flex items-center px-6 py-5 border-b-2 border-pink-50 bg-white shadow-sm">
            <h3 className="flex-1 text-xl font-black text-[#5C3D3D] text-center tracking-tight">
          ...
          <div
            ...
            className="flex-1 overflow-y-auto p-6 flex flex-col gap-6 bg-[#FFF9F0]"
          >
            {errorMsg && (
              <div
                ref={errorRef}
            ...
            <section>
              <h4 className="font-bold text-[#5C3D3D] mb-3 text-base">
              ...
              <div className="space-y-3">
                ...
                <div className="flex gap-3">
                  <input
                    ...
                    className="flex-1 border-2 border-pink-100 rounded-xl px-4 py-2.5 text-sm bg-white focus:ring-0 focus:border-pink-400 outline-none font-medium text-[#4A3732]"
                  />
                  <input
                    type="time"
                    required
              ...
              <div className="space-y-3">
                {flavors.map((flavor) => {
                  ...
                  return (
                    <div
                      key={flavor.id}
                      className="flex items-center justify-between bg-white p-3 border-2 border-pink-50 rounded-xl shadow-sm"
                    >
                      <div>
                        <p className="font-bold text-[#5C3D3D] text-sm">
                          {flavor.name}
                        </p>
                        <p className="text-pink-500 font-bold text-xs mt-0.5">
                          {formatVND(flavor.pricePerSet)}/set
                        </p>
                      </div>
                      <div className="flex items-center gap-3">
                        <button
                          ...
                          disabled={qty === 0}
                          className={`w-8 h-8 rounded-lg flex items-center justify-center font-black text-lg shadow-sm transition ${qty === 0 ? "bg-gray-50 text-gray-300" : "bg-pink-100 text-pink-600 active:bg-pink-200"}`}
                        >
                          -
                        </button>
                        <span className="w-5 text-center font-black text-base text-[#5C3D3D]">
                          {qty}
                        </span>
                        <button
                          type="button"
                          onClick={() => handleQtyChange(flavor.id, 1)}
                          className="w-8 h-8 rounded-lg flex items-center justify-center font-black text-lg bg-pink-400 text-white shadow-sm active:bg-pink-500 transition"
                        >
                          +
                        </button>
                      </div>
          ...
          <div className="p-4 bg-white border-t-2 border-pink-50 pb-safe shadow-[0_-10px_30px_rgba(252,231,243,0.4)]">
            <div className="flex justify-between items-center mb-4">
              ...
              <span className="text-xl font-black text-[#5C3D3D]">
            ...
            <div className="flex gap-3">
              <button
                ...
                className="px-5 py-3 text-sm text-pink-500 font-bold bg-pink-50 rounded-xl active:bg-pink-100 transition"
              >
                Hủy
              </button>
              <button
                ...
                className="flex-1 py-3 text-sm text-white font-bold bg-pink-400 rounded-xl shadow-lg shadow-pink-200 active:opacity-90 transition"
              >
                Lưu Đơn Hàng
              </button>
            </div>
